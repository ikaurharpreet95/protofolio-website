...
const info = [
  ...
  { icon: Phone, label: "Phone", value: "+1 (984) 302 3290", href: "tel:+19843023290" },
  ...
  return (
    <section id="contact" className="py-24 md:py-32 bg-gradient-to-b from-ivory to-rose/10">
      <div className="max-w-6xl mx-auto px-6 lg:px-10">
        <SectionHeading eyebrow="Contact" title="Let's Create Something Beautiful Together" subtitle="Tell me about your celebration — I'd love to design an invitation that feels truly yours." />

        <div className="mt-14 grid lg:grid-cols-5 gap-10">
          <form onSubmit={handleSubmit(onSubmit)} className="lg:col-span-3 glass rounded-3xl p-8 md:p-10 shadow-soft space-y-5" noValidate>
            <div className="grid sm:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <Select value={watch("eventType") ?? ""} onValueChange={(v) => setValue("eventType", v, { shouldValidate: true })}>
            ...
            <button type="submit" disabled={isSubmitting} className="w-full rounded-full bg-gold text-ivory py-4 font-medium tracking-wide hover:bg-gold/90 transition shadow-luxe disabled:opacity-60">
              ...
              <ul className="space-y-5">
                {info.map((i) => (
                  <li key={i.label} className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-[10px] uppercase tracking-[0.25em] text-ivory/50">{i.label}</p>
                      {i.href ? <a href={i.href} className="text-sm hover:text-gold transition">{i.value}</a> : <p className="text-sm">{i.value}</p>}
                    </div>
                  </li>
                ))}
              </ul>
              <div className="mt-8 pt-6 border-t border-ivory/15">
                <p className="text-[10px] uppercase tracking-[0.25em] text-ivory/50 mb-3">Follow along</p>
                <div className="flex gap-3">
                  {socials.map((s) => (
                    <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label} className="w-10 h-10 rounded-full border border-ivory/20 flex items-center justify-center hover:bg-gold hover:border-gold hover:text-charcoal transition">
                      <s.icon size={16} />
                    </a>
                  ))}
                </div>
