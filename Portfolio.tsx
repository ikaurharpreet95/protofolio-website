  ...
  return (
    <section id="portfolio" className="py-24 md:py-32 bg-gradient-to-b from-ivory to-muted/40">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <SectionHeading
          eyebrow="Portfolio"
          title="Selected Invitation Work"
          subtitle="A growing collection of bespoke invitation designs crafted for life's most cherished moments."
        />
          ...
          {categories.map((c) => (
            <button
              ...
              className={cn(
                ...
                active === c
                  ? "bg-gold text-ivory border-gold shadow-soft"
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((p, i) => (
              <motion.article
                ...
                className="group relative rounded-3xl overflow-hidden bg-card shadow-soft hover:shadow-luxe transition-all duration-500"
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-charcoal/85 via-charcoal/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-7">
                  ...
                  <button
                    ...
                    className="mt-5 inline-flex items-center gap-2 self-start rounded-full bg-gold/95 text-ivory px-5 py-2 text-xs font-medium tracking-wider uppercase hover:bg-gold transition"
                  >
          ...
          {current && (
            <>
              ...
              <img
                src={current.image}
                alt={current.title}
                className="w-full h-auto max-h-[75vh] object-contain bg-charcoal/5"
              />
              <div className="p-6">
                <p className="text-[10px] uppercase tracking-[0.3em] text-gold">
                  {current.category}
